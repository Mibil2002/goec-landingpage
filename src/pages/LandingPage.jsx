...
const MultistepModal = ({ isOpen, onClose }) => {
    ...
    const getInvestmentAmount = () => {
        if (formData.state === 'Kerala') return '₹35';
        if (formData.state === 'Karnataka') return '₹50 - ₹55';
        return '₹35'; // default
    };

    const handleInvestmentInterest = (val) => {
        setFormData({ ...formData, investment_interest: val });
        if (val === 'No') {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-2xl rounded-md p-8 md:p-10 shadow-2xl flex flex-col min-h-[450px]">
                ...
                <div className="mb-8 pr-12 flex justify-between items-end">
                    <div className="w-full">
                        <div className="flex justify-between items-center mb-3">
                            <h3 className="text-[#5c5e62] font-medium text-sm">Step {step} of 4</h3>
                            <span className="text-[#11EAAD] text-xs font-bold px-3 py-1.5 bg-[#11EAAD]/10 rounded-full flex items-center gap-1.5">
                                <Zap className="w-3.5 h-3.5" fill="currentColor" /> Takes 60 seconds
                            </span>
                        </div>
                ...
                <div className="flex-grow flex flex-col justify-center">
                            ...
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="relative">
                                        <select name="state" value={formData.state} onChange={handleChange} required className="w-full bg-[#f4f4f4] border border-transparent rounded-md px-4 py-3 text-[#171a20] focus:outline-none focus:bg-white focus:border-[#11EAAD] focus:ring-1 focus:ring-[#11EAAD] transition-all duration-300 appearance-none cursor-pointer hover:bg-[#e8e8e8]">
                    ...
                    {step === 2 && (
                        <div className="space-y-6 animate-in fade-in duration-300">
                            <label className="text-[#171a20] text-2xl md:text-3xl font-semibold block leading-tight tracking-tight">
                                Our investment plan starts from {getInvestmentAmount()} lakhs. Are you interested in proceeding?
                            </label>
                            <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                {['Yes', 'No'].map(val => (
                                    <label key={val} className="flex-1 cursor-pointer">
                                        <input type="radio" name="investment_interest" value={val} checked={formData.investment_interest === val} onChange={() => handleInvestmentInterest(val)} className="peer sr-only" required />
                ...
                <div className="flex justify-between pt-8 mt-8 gap-4">
                    ...
                    {step < 4 && step !== 2 ? (
                        <Button onClick={handleNext} disabled={
                            (step === 1 && (!formData.name || !formData.phone || !formData.city || !formData.state || !formData.email)) ||
                            (step === 3 && !formData.own_land)
                        ...
                        <Button onClick={handleSubmit} disabled={isSubmitting || !formData.site_visit} className="bg-[#11EAAD] hover:bg-[#0447B8] text-[#171a20] hover:text-white font-semibold px-10 py-6 text-base rounded-md transition-all duration-300 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center active:scale-95">
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-[#171a20] font-sans selection:bg-[#3e6ae1]/20">
      ...
      {/* Trust Banner */}
      <div className="fixed top-0 w-full bg-[#11EAAD] text-[#171a20] text-xs sm:text-sm font-semibold py-2 px-4 text-center tracking-wide flex justify-center items-center gap-4 sm:gap-8 z-[60]">
          <span className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4" /> ISO Certified</span>
          <span className="hidden sm:inline-block w-1 h-1 bg-[#171a20] rounded-full"></span>
          <span className="flex items-center gap-1.5"><TrendingUp className="w-4 h-4" /> Assured Returns</span>
          <span className="hidden sm:inline-block w-1 h-1 bg-[#171a20] rounded-full"></span>
          <span className="flex items-center gap-1.5"><Settings className="w-4 h-4" /> Turnkey Implementation</span>
      </div>
      {/* Navbar - Clean, switches from transparent to white */}
      <nav className={`fixed w-full z-50 top-8 sm:top-[36px] transition-all duration-300 ${scrolled ? 'bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)] py-3' : 'bg-transparent py-5'}`}>
      ...
      <section className="py-10 sm:py-16 bg-white border-b border-[#e2e3e3]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 divide-y divide-[#e2e3e3] sm:divide-y-0 sm:divide-x">
                  <div className="text-center py-4 px-2">
                      <div className="text-5xl sm:text-6xl font-bold text-[#171a20] mb-2 tracking-tight">
                          <AnimatedCounter end={10} suffix="%+" />
                      </div>
                      <div className="text-[#5c5e62] text-sm sm:text-base font-semibold uppercase tracking-wider">Assured ROI</div>
                  </div>
                  <div className="text-center py-4 px-2">
                      <div className="text-5xl sm:text-6xl font-bold text-[#171a20] mb-2 tracking-tight">
                          <AnimatedCounter end={9} suffix="+" />
                      </div>
                      <div className="text-[#5c5e62] text-sm sm:text-base font-semibold uppercase tracking-wider">Cities Covered</div>
                  </div>
                  <div className="text-center py-4 px-2">
                      <div className="text-5xl sm:text-6xl font-bold text-[#171a20] mb-2 tracking-tight">
                          <AnimatedCounter end={500} suffix="+" />
                      </div>
                      <div className="text-[#5c5e62] text-sm sm:text-base font-semibold uppercase tracking-wider">Charging Points</div>
                  </div>
                  <div className="text-center py-4 px-2">
                      <div className="text-5xl sm:text-6xl font-bold text-[#171a20] mb-2 tracking-tight">
                          <AnimatedCounter end={300} suffix="+" />
                      </div>
                      <div className="text-[#5c5e62] text-sm sm:text-base font-semibold uppercase tracking-wider">Investors</div>
                  </div>
              </div>
          </div>
      </section>
                ...
                <div className="mt-10 p-6 bg-gradient-to-br from-white/10 to-white/5 border border-[#11EAAD]/40 rounded-xl backdrop-blur-lg relative overflow-hidden group hover:border-[#11EAAD] transition-all cursor-pointer shadow-2xl hover:shadow-[0_10px_30px_rgba(17,234,173,0.15)] block translate-y-4" onClick={() => setIsModalOpen(true)}>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
                        <div>
                            ...
                            <h3 className="text-white text-2xl font-medium tracking-tight">Assured <br className="hidden sm:block md:hidden"/><span className="text-[#11EAAD] font-semibold text-3xl">10%+ ROI</span></h3>
                        </div>
      ...
      <section className="py-14 sm:py-24 px-4 sm:px-6 bg-[#f4f4f4]" id="models">
         <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6 sm:gap-8 max-w-7xl mx-auto">
                {/* FOCO */}
                <div className="bg-white rounded-md p-6 sm:p-10 flex flex-col shadow-sm border border-[#e2e3e3] hover:border-[#11EAAD] transition-colors duration-300">
                ...
                {/* I COCO */}
                <div className="bg-white rounded-md p-6 sm:p-10 flex flex-col shadow-sm border border-[#e2e3e3] hover:border-[#11EAAD] transition-colors duration-300">
                    <Users className="w-10 h-10 text-[#11EAAD] mb-6" strokeWidth={1.5}/>
                    <h3 className="text-2xl font-medium text-[#171a20] mb-2 tracking-tight">I COCO Model</h3>
                    <p className="text-[#5c5e62] text-xs font-semibold uppercase tracking-wide mb-6">Investor Owned, Company Operated</p>
                    <p className="text-[#393c41] text-base leading-relaxed mb-10 flex-grow">
                        Perfect for pure investors. You provide the capital investment for the charging station, and GO EC handles the location sourcing, installation, and complete day-to-day operations.
                    </p>
                    <Button onClick={() => setIsModalOpen(true)} className="w-full bg-[#11EAAD] text-[#171a20] hover:bg-[#0447B8] hover:text-white py-6 rounded font-semibold transition-all duration-300 active:scale-95 shadow-sm">
                        Invest in I COCO
                    </Button>
                </div>
            </div>
      ...
      <section className="py-14 sm:py-24 bg-white border-t border-[#e2e3e3]" id="stories">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-8 sm:mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <a href="https://www.youtube.com/@goecevcharging" target="_blank" rel="noopener noreferrer">
                <Button className="bg-[#ff0000] text-white hover:bg-[#cc0000] font-semibold px-6 py-4 flex items-center gap-2 rounded">
                    <PlayCircle className="w-5 h-5" /> Visit our YouTube Channel
                </Button>
            </a>
         </div>
         {/* Removing horizontal scroll with iframes to remove outbound links completely */}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
            ...
            <div className="border-t border-[#e2e3e3]">
                <FAQItem 
                    ...
                    answer="The minimum investment varies by region. For Kerala, it starts at ₹35 Lakhs. For Karnataka, the investment ranges from ₹50 Lakhs to ₹55 Lakhs." 
                />
                <FAQItem 
